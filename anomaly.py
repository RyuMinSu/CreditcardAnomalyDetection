...
Conn = dbConn.connect()
df.to_sql(name="creditcard", con=dbConn, if_exists="fail", index=False)
print("db저장 완료")


#%%
conn = pymysql.connect(host="localhost", user="root", passwd="123123", db="anomalydetection", charset="utf8")
cur = conn.cursor()
df = pd.read_sql(sql="select * from creditcard", con=conn)
df.shape

#%%
report = create_report(df)
report.save('creditcard.html')
...
plt.title('original df correlation heatmap')
sns.heatmap(df.corr(), cmap = 'coolwarm_r', mask=mask, linewidths=0.5)
plt.show()

plt.figure(figsize=(15,7))
plt.title('new df correlation heatmap')
sns.heatmap(newDf.corr(), cmap = 'coolwarm_r', mask=mask, linewidths=0.5)
plt.show()
